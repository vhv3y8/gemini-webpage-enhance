export class ScrollNavigator {
  ...
  private checkAndInject() {
    ...
    if (
      ...
      this.navElement &&
      this.navElement.parentElement &&
      document.body.contains(this.navElement)
    ) {
  ...
  private updateQueriesAndPositions() {
    ...
    const currentQueries = Array.from(
      this.container.querySelectorAll("user-query")
    ) as HTMLElement[]

    // Check if query list actually changed to prevent excessive DOM repaints
    const listChanged =
      currentQueries.length !== this.queries.length ||
  ...
  private rebuildDots() {
    ...
    this.queries.forEach((uq, index) => {
      ...
      const cleanText =
        uqClone.textContent?.replace(/\s+/g, " ").trim() || `Chat ${index + 1}`
      ...
      dot.style.background =
        "var(--fallback-color-on-surface, rgba(128, 128, 128, 0.45))"
  ...
  private repositionDots() {
    ...
    const dotWrappers = Array.from(
      this.navElement.querySelectorAll(".scroll-dot-wrapper")
    ) as HTMLElement[]
  ...
  private handleScroll = () => {
    ...
    for (let i = 0; i < this.queries.length; i++) {
      ...
      const offsetTop =
        uq.getBoundingClientRect().top - containerRect.top + scrollTop
